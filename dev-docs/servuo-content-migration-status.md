...
- [ ] **`SayTo` hue overload** — *(Astronomy)* Edge `BaseVendor.SayTo` has no text-hue overload, so `Willebrord`'s "Star Charts only" line dropped ServUO's `1163` hue. **Close:** add a hued `SayTo`/`SayTo`-equivalent, or accept default hue.
